...
```
trip-assistant/
├── core/          Pure Kotlin/JVM. No Android. 89 unit tests, runs in about a second.
...
Standard Gradle/AGP project: Kotlin 2.0.21, AGP 8.5.2, compileSdk 35, minSdk 26, Jetpack Compose,
Room, Hilt, ML Kit Text Recognition, Play Billing.
...
| | |
|---|---|
| `:core` — parsing, validation, economics, rules, entitlement | **Compiled; 89 unit tests pass.** Run `./gradlew :core:test`. |
...
* **`ParserSamples.kt`** — the permanent sample library (spec section 55). Each entry is the
  ...
  covering the inline layout, the stacked layout, an unlabelled compact card, kilometres, damaged
